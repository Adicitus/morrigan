...
if ("api" -notin $u.identity.functions) {
    Invoke-RestMethod -Uri "http://localhost/auth/identity/$($u.identity.id)" -Method Patch -Headers $headers -Body (@{functions = ($u.identity.functions + "api")} | ConvertTo-Json) -ContentType 'application/json'
