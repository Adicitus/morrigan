...
$t = Invoke-RestMethod -Uri "$hostname/api/auth" -Body $authBody -Method Post -ContentType application/json
$headers = @{ authorization="bearer $($t.token)" }

$u = Invoke-RestMethod -Uri "$hostname/api/auth/identity/me" -Method Get -Headers $headers
...
if ("api" -notin $u.identity.functions) {
    ...
    Invoke-RestMethod -Uri "$hostname/api/auth/identity/$($u.identity.id)" -Method Patch -Headers $headers -Body $functionPatchBody -ContentType 'application/json'

    $t = Invoke-RestMethod -Uri "$hostname/api/auth" -Body $authBody -Method Post -ContentType application/json
    $headers = @{ authorization="bearer $($t.token)" }
}

$clientEndpoint = "$hostname/api/core/client"
